	...
	ConstPMoaMmPlayerStateInfo	pPlayerStateInfo, PIMoaMmGC pGC)
{
...
#ifdef _IOS
    {
        if( nativeInfo.type == kMoaMmGCType_CGContext )
        {
            // NON-DTS Mode
            CGContextRef context = (CGContextRef)nativeInfo.data.ios_cgcontext.cgContextRef;
            ...
            switch (pObj->state)
            {
                case kMyActorState_X:
                {
                    ...
                    CGContextMoveToPoint(context, pImageRect->left+PENSIZE, pImageRect->top+PENSIZE);
                    CGContextAddLineToPoint(context, pImageRect->right-PENSIZE, pImageRect->bottom-PENSIZE);
                    CGContextMoveToPoint(context, pImageRect->left+PENSIZE, pImageRect->bottom-PENSIZE);
                    CGContextAddLineToPoint(context, pImageRect->right-PENSIZE, pImageRect->top+PENSIZE); 
                ...
                {                
                    ...
                    CGRect rect;
                    MoaRectToCGRect(pImageRect, &rect);
                    rect = CGRectInset(rect, ovalSize+PENSIZE, ovalSize+PENSIZE);
                    CGContextStrokeEllipseInRect(context, rect);                   
                }
            }
            CGContextRestoreGState(context);
        }
        else if(nativeInfo.type == kMoaMmGCType_IOSView)
        {
            // DTS Mode
        }
    }
...
#ifdef CALAYER_AVAILABLE		
	CGLContextObj 		savedContext, drawContext;
#endif
	PenState			savePen;
	RGBColor			saveColor;
	...
	switch (nativeInfo.type)
	{
		...
		case kMoaMmGCType_MacGWorld:
			...
			break;
#ifdef CALAYER_AVAILABLE
		case kMoaMmGCType_MacCALayer:
			savedContext  =  CGLGetCurrentContext();	
			drawContext = (CGLContextObj)nativeInfo.data.mac_calayer.calayerPtr;
			if(drawContext)
			{
				CGLSetCurrentContext(drawContext);
			}
#endif	
		default:
	...
	if(nativeInfo.type == kMoaMmGCType_MacGWorld || nativeInfo.type == kMoaMmGCType_MacWindow)
	{
		GetPort(&savePort);
		SetPort( drawPort );
...
#ifdef CALAYER_AVAILABLE		
	else if(nativeInfo.type == kMoaMmGCType_MacCALayer)
	{
		glColor3i((GLint)myColor.red, (GLint)myColor.green, (GLint)myColor.blue);
		glLineWidth(PENSIZE);
	}
#endif
	switch (pObj->state)
	{
		case kMyActorState_X:
		{
			...
			{
				glBegin(GL_LINES);
				glVertex2d(pImageRect->left+PENSIZE, pImageRect->top+PENSIZE);
				glVertex2d(pImageRect->right-PENSIZE, pImageRect->bottom-PENSIZE);
				glVertex2d(pImageRect->left+PENSIZE, pImageRect->bottom-PENSIZE);
				glVertex2d(pImageRect->right-PENSIZE, pImageRect->top+PENSIZE);
				glEnd();
			}
#endif			
			break;
		...
		case kMyActorState_Oval:
		{
			...
			{
				MoaLong centerX ,centerY;
				centerX = (pImageRect->right - pImageRect->left)/2;
				centerY = (pImageRect->bottom - pImageRect->top)/2;
				glBegin(GL_LINES);
				for (int i = 0; i < 180; i++)
				{
					float x = ovalSize * cos(i) - centerX;
					float y = ovalSize * sin(i) + centerY;
					glVertex3f(x + centerX, y - centerY,0);
					
					x = ovalSize * cos(i + 0.1) - centerX;
					y = ovalSize * sin(i + 0.1) + centerY;
					glVertex3f(x + centerX,y - centerY,0);
				}
				glEnd();
			}	
