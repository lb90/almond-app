...
#elif defined(_IOS)
	if (nativeGCInfo.type == kMoaMmGCType_IOSView)
    {
        CGRect rct = [(UIView*)nativeGCInfo.data.ios_view.iosViewPtr bounds];
        CGRectToMoaRect(&rct, &stageRect);
    }
#endif     
...
#if defined(WINDOWS) || defined(_IOS)
