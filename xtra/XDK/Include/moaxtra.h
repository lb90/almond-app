...
/* ========================================================================== */
	...
	*/
	...
	#ifndef MOA_DO_NOT_DEFINE_REFIID
...
#else
    EXTERN_C MoaFileRef DLL(gXtraFileRef);
#endif
#endif
...
#define _GLUE_GETCLASSFORM \
	typedef struct { \
		...
		MoaDestroyedProc * pDestroyProc; \
		MoaBool found; \
	} DLL(_XtraGetClassFormRec); \
	STDMETHODIMP DLL(_XtraGetClassForm)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, DLL(_XtraGetClassFormRec) * pInfo); \
	STDMETHODIMP DLL(_XtraGetClassForm)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, DLL(_XtraGetClassFormRec) * pInfo) \
	{ \
		pClassID; classVersion; \
		*pInfo->pObjSize = objSize; \
...
#define _GLUE_GETINTERFACE \
	typedef struct { \
		ConstPMoaInterfaceID pInterfaceID; \
		XtraBuildInterfaceProc buildProc; \
	} DLL(_XtraGetClassInterfaceRec); \
	STDMETHODIMP DLL(_XtraGetClassInterface)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, DLL(_XtraGetClassInterfaceRec) * pInfo); \
	STDMETHODIMP DLL(_XtraGetClassInterface)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, DLL(_XtraGetClassInterfaceRec) * pInfo) \
	{ \
		pClassID; classVersion; interfaceVersion; \
		if (MoaEqualID(pInterfaceID, pInfo->pInterfaceID)) { \
	...
	STDAPI DLL(GetInterface)(PMoaVoid pObj, ConstPMoaClassID pClassID, ConstPMoaInterfaceID pInterfaceID, PPMoaVoid ppv); \
	STDAPIIMP DLL(GetInterface)(PMoaVoid pObj, ConstPMoaClassID pClassID, ConstPMoaInterfaceID pInterfaceID, PPMoaVoid ppv) { \
		DLL(_XtraGetClassInterfaceRec) info; \
		X_ENTER_DLLENTRYPOINT \
		info.pInterfaceID = pInterfaceID; \
		info.buildProc = NULL; \
		DLL(SecretGlue)(pClassID, NULL, (XtraEnumClassInterfacesProc)DLL(_XtraGetClassInterface), &info); \
...
#define _GLUE_GETCLASSINFO \
	typedef struct { \
		MoaLong count; \
		PMoaClassInterfaceInfo pCur; \
	} DLL(_XtraGetClassInfoRec); \
	STDMETHODIMP DLL(_XtraGcfInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, DLL(_XtraGetClassInfoRec) * pInfo); \
	STDMETHODIMP DLL(_XtraGcfInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, MoaLong objSize, MoaCreatedProc createProc, MoaDestroyedProc destroyProc, DLL(_XtraGetClassInfoRec) * pInfo) \
	{ \
		objSize; createProc; destroyProc; \
		++pInfo->count; \
	...
	STDMETHODIMP DLL(_XtraGciInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, DLL(_XtraGetClassInfoRec) * pInfo); \
	STDMETHODIMP DLL(_XtraGciInfo)(ConstPMoaClassID pClassID, MoaLong classVersion, ConstPMoaInterfaceID pInterfaceID, MoaLong interfaceVersion, XtraBuildInterfaceProc buildProc, DLL(_XtraGetClassInfoRec) * pInfo) \
	{ \
		buildProc; \
		++pInfo->count; \
...
#ifdef _IOS
#define BEGIN_DECLARE_IOS_XTRA 

#define END_DECLARE_IOS_XTRA \
EXTERN_C PMoaVoid DLL(_MBNew)(size_t size, PIMoaCalloc pCalloc); \
EXTERN_C void DLL(_MBDelete)(PMoaVoid ptr, PIMoaCalloc pCalloc); \
EXTERN_C MoaLong DLL(gXtraInterfaceCount);
#endif

// helper macro to register each given Xtra
#define DECLARE_REGISTER_XTRAS \
MoaError DLL(MoaRegisterExtras) (MoaContext context); \
EXTERN_XTRA_ENTRIES \
MoaError DLL(MoaRegisterExtras) (MoaContext context) \
{ \
return MoaRegisterMemXtra(context, \
(MoaGetClassObjectProc)DLL(GetClassObject), \
(MoaGetInterfaceProc)DLL(GetInterface), \
(MoaGetClassFormProc)DLL(GetClassForm), \
(MoaGetCanUnloadProc)DLL(CanUnloadNow), \
(MoaGetClassInfoProc)DLL(GetClassInfo)); \
} \

#define CALL_REGISTER_XTRAS(name) \
do { \
MoaError ErrFromMoa; \
\
extern MoaError name##MoaRegisterExtras(MoaContext context); \
ErrFromMoa = name##MoaRegisterExtras(context); \
if (ErrFromMoa) \
return ErrFromMoa; \
} while(0);\
